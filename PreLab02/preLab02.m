...
title('X_1 | X_1 ~ N(\mu=10, \sigma=100)')
...
title('X_1+X_2 | X_1,X_2 ~ U(a=0, b=1)')

% Part c
subplot(3, 1, 3)
x4 = sum(rand(100000, 4), 2);
histogram(x4, 100, 'Normalization', 'pdf');
title('X_1+X_2+X_3+X_4 | X_1,X_2,X_3,X_4 ~ U(a=0, b=1)')
...
subplot(2, 2, 1)
plot(real(rx_smpl))
title('rx-smpl real part');

subplot(2, 2, 2)
plot(imag(rx_smpl))
title('rx-smpl imaginary part');

subplot(2, 2, 3)
plot(real(hdr_smpl))
title('hdr-smpl real part');

subplot(2, 2, 4)
plot(imag(hdr_smpl))
title('hdr-smpl imaginary part');
